      ...
      IObservableParser<TSource, TSource> close)
    {
      ...
      return Observable.Create<IParseResult<IObservable<TSource>>>(
        observer =>
        {
          ...
          disposables.Add(open.Parse(source).Subscribe(
            openResult =>
            {
              hasOpenResult = true;
              ...
              recursion.Disposable = Scheduler.Immediate.Schedule(
                self =>
                {
                  ...
                  innerSubscription.Disposable = innerParser.Parse(capturedBranch).Subscribe(
                    _ => { },
                    observer.OnError,
                    () =>
                    {
                      ...
                      else
                      {
                        ...
                        if (hasInnerResult)
                        {
                          bufferedResults.Concat().ForEach(observer);
                        }
                        else
                        {
