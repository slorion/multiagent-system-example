  ...
  public static partial class Observable2
  {
      ...
      this IObservable<TSource> source)
    {
      ...
      return from value in source
             where value != null
             from change in FromPropertyChangedPattern(value)
             select change;
    }
      ...
      Func<TSource, IObservable<TUntil>> takeUntilSelector)
    {
      ...
      return from value in source
             where value != null
             from change in FromPropertyChangedPattern(value).TakeUntil(takeUntilSelector(value))
             select change;
    }

    /// <summary>
    /// Returns a flattened observable sequence of property changed notifications for every existing, added and replacement
    /// element in the specified <paramref name="source"/> sequence of collection notifications.
    /// </summary>
    /// <remarks>
    /// <para>
      ...
      this IObservable<CollectionNotification<TSource>> source)
    {
      ...
      return source.PropertyChanges(value => (IObservable<TSource>)Scalar.Return(value));
    }
      ...
      IEqualityComparer<TSource> comparer)
    {
      ...
      return source.PropertyChanges(value => (IObservable<TSource>)Scalar.Return(value), comparer);
    }

    /// <summary>
    /// Returns a flattened observable sequence of property changed notifications for every existing, added and replacement
    /// element in the specified <paramref name="source"/> sequence of collection notifications and dependent objects returned
    /// by the specified <paramref name="selector"/> function.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// returned by the specified <paramref name="selector"/> function.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Property changed notifications for a given object are ignored after receiving a collection notification indicating 
    ...
    /// notifications returned by the specified <paramref name="selector"/> function.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Property changed notifications for a given object are ignored after receiving a collection notification indicating 
      ...
      IEqualityComparer<TSource> comparer)
    {
      ...
      return source.Publish(published =>
        ...
        let dependencyChanges = intermediateSelector(value).Publish(dependenciesPublished =>
          dependencySelector(dependenciesPublished).PropertyChanges(dependency =>
            valueReleased.Merge(dependencyChangesUntilSelector(dependenciesPublished, dependency))))
        let notifier = selector(value)
        from change in
          notifier == null
          ? dependencyChanges
          : Scalar.Return(notifier).PropertyChanges(_ => valueReleased).Merge(dependencyChanges)
        select change);
    }
  }
}